...
#define B_FLAG_DYNAMAX_BATTLE       FLAG_CITIES_DYNAMAX_BATTLE // If this flag is set, the ability to Dynamax in battle is enabled for all Trainers.
#define B_FLAG_TERA_ORB_CHARGED     FLAG_CITIES_TERA_ORB_CHARGED // If this flag is set, the Tera Orb is charged. It is automatically set upon healing and cleared upon Terastallizing once configured.
...
#define B_VAR_DIFFICULTY            VAR_CITIES_DIFFICULTY // If not 0, you can use this var to control which difficulty version of a Trainer is loaded. This should be manually set by the developer using Script_SetDifficulty AFTER NewGameInitData has run.
...
#define B_SHOW_EFFECTIVENESS        SHOW_EFFECTIVENESS_SEEN // If not SHOW_EFFECTIVENESS_NEVER, the PP string is replaced by a type effectiveness indicator based on the move and the target. Cities of Emerald: battle_controller_player.c overrides this at runtime via CitiesShowEffectiveness().
